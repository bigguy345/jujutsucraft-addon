...
public class Config {
    ...
    public static ForgeConfigSpec.DoubleValue MAX_DASH_LEVEL;
    public static ForgeConfigSpec.DoubleValue MAX_DASH_STRENGTH;
    ...
    static {
        ...
        BUILDER.push("Reversed Cursed Technique Others");
        MAX_RCT_LEVEL = BUILDER.comment("Max RCT level").defineInRange("maxRCTLLevel", 100f, 0, 1000);
        MAX_RCT_LEVEL_AMPLIFIER = BUILDER.comment("RCT effect amplifier at max level (linearly interpolates between 0 and this value depending on level to max level ratio)").defineInRange("maxRCTAmplifier", 20, 0, 99);
        BUILDER.pop();

        BUILDER.push("Dash");
        MAX_DASH_LEVEL = BUILDER.comment("Max Dash level").defineInRange("maxDashLevel", 100f, 0, 1000);
        MAX_DASH_STRENGTH = BUILDER.comment("Strength of fully charged Dash at max level (linearly interpolates between 0 and this value depending on level to max level ratio)").defineInRange("maxDashStrength", 20, 0, 50f);
